...
function showError(
  ...
  password1Error: HTMLSpanElement
) {
  if (password1.validity.valueMissing) {
    ...
    password1Error.textContent =
      'Password must be minimum 8 characters long and contain at least one number, one lowercase and one uppercase characters with no whitespace';
    applyStyleToInput(password1, 'invalid');
  }
}
...
export function setPasswordValidityListener(element: HTMLElement) {
  ...
  const passwordError = element.querySelector(
    `.password1 ~ span.validation-message`
  ) as HTMLSpanElement;
...
export function setPasswordVisibility(number: 1 | 2, element: HTMLElement) {
  const eye = element.querySelector(
    `.password${number} + img`
  ) as HTMLImageElement;
...
export function setPasswordMatchCheck(element: HTMLElement) {
  ...
  const password2Error = element.querySelector(
    `.password2 ~ span.validation-message`
  ) as HTMLSpanElement;
